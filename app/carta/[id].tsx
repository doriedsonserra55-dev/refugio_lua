...
export default function LetterDetailScreen() {
  ...
  if (!letter) return <View style={styles.missing}><Text style={styles.missingText}>Esta carta não está mais disponível.</Text><PrimaryButton label="Voltar ao Mural" onPress={() => router.replace("/mural" as never)} /></View>;
  ...
  return (
    <View style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={styles.topbar}><Pressable onPress={() => router.replace("/mural" as never)} style={({ pressed }) => [styles.back, pressed && styles.pressed]}><MaterialCommunityIcons name="arrow-left" size={22} color="#2F6F8F" /></Pressable><Text style={styles.topTitle}>Leitura com cuidado</Text><View style={{ width: 42 }} /></View>
