...
export async function apiCall<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  // Supabase access tokens are valid on both native and web. Keep cookies
  // enabled for legacy sessions and iframe previews, but prefer the current
  // Supabase token whenever one exists so profile sync works on web too.
